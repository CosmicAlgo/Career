...
const APPLICATION_STATUSES = [
  { id: 'interested',   label: 'Interested',   color: 'text-muted-foreground',   border: 'border-border'        },
  { id: 'applied',      label: 'Applied',       color: 'text-amber-500',           border: 'border-amber-500/30'  },
  { id: 'phone_screen', label: 'Phone Screen',  color: 'text-violet-400',          border: 'border-violet-400/30' },
  { id: 'technical',    label: 'Technical',     color: 'text-blue-400',            border: 'border-blue-400/30'   },
  { id: 'final',        label: 'Final Round',   color: 'text-emerald-400',         border: 'border-emerald-400/30'},
  { id: 'offer',        label: 'Offer',         color: 'text-green-500',           border: 'border-green-500/30'  },
  { id: 'rejected',     label: 'Rejected',      color: 'text-red-500',             border: 'border-red-500/30'    },
];
...
export default function ApplicationsPage() {
  ...
  const [formData, setFormData] = useState<CreateApplicationRequest>(EMPTY_FORM);
        ...
        <main className="max-w-7xl mx-auto px-4 py-8">
          ...
          {stats && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              {[
                { label: `Total (${daysAnalyzed}d)`, value: stats.total_applications, color: 'text-foreground' },
                { label: 'Response Rate', value: `${stats.response_rate.toFixed(1)}%`, color: 'text-green-500' },
                { label: 'Interview Rate', value: `${stats.interview_rate.toFixed(1)}%`, color: 'text-blue-400' },
                { label: 'Offer Rate', value: `${stats.offer_rate.toFixed(1)}%`, color: 'text-violet-400' },
              ].map(stat => (
          ...
          {followUps.length > 0 && (
            <div className="mb-6 p-4 rounded-lg border border-amber-500/30 bg-amber-500/5 flex items-center gap-3">
              ...
              <p className="text-sm font-mono text-amber-500">{followUpMessage}</p>
            </div>
          )}
          ...
          <div className="flex gap-4 overflow-x-auto pb-4">
            {APPLICATION_STATUSES.map(status => (
              <div key={status.id} className={`min-w-[240px] w-64 shrink-0 border rounded-xl bg-card flex flex-col ${status.border}`}>
                <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                  <span className={`text-xs font-mono font-semibold uppercase tracking-wider ${status.color}`}>
                    {status.label}
                  </span>
                ...
                <div className="p-3 flex flex-col gap-2 flex-1 min-h-[120px]">
                  {applicationsByStatus[status.id]?.map(app => (
                    ...
                    >
                      ...
                      <div className="text-sm font-mono font-semibold text-foreground mb-1 pr-5 truncate">{app.job_title}</div>
                      <div className="text-xs font-mono text-primary mb-2">{app.company}</div>
                      {app.location && <div className="text-[10px] font-mono text-muted-foreground mb-1">📍 {app.location}</div>}
                      {app.salary_range && <div className="text-[10px] font-mono text-green-500 mb-1">💰 {app.salary_range}</div>}
                      {app.date_applied && <div className="text-[10px] font-mono text-muted-foreground">{new Date(app.date_applied).toLocaleDateString()}</div>}
          ...
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={closeForm} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-all">
                  <X className="h-5 w-5" />
                </button>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div>
                <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider mb-1.5">Job Title *</label>
                <input
                  type="text" required
              ...
              <div>
                <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider mb-1.5">Company *</label>
                <input
                  type="text" required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider mb-1.5">Source</label>
                  <select
                  ...
                  >
                    {APPLICATION_SOURCES.map(source => (
                      <option key={source} value={source}>{source.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider mb-1.5">Status</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider mb-1.5">Location</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider mb-1.5">Salary Range</label>
                  <input
                    type="text"
              ...
              <div className="flex gap-3 justify-end pt-2">
                <button type="button" onClick={closeForm} className="px-4 py-2 rounded-lg border border-border bg-transparent text-muted-foreground font-mono text-sm hover:text-foreground hover:bg-muted transition-all">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2 rounded-lg border border-primary/30 bg-primary/10 text-primary font-mono text-sm font-semibold hover:bg-primary/20 transition-all">
