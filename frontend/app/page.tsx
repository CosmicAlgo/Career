      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <main className="max-w-7xl mx-auto px-4 py-8 w-full">
          ...
          <div className="p-6 rounded-xl border border-primary/30 bg-primary/5 glass-panel">
            <p className="text-base text-primary font-mono mb-2">No pipeline data yet</p>
            <p className="text-sm text-muted-foreground font-mono mb-4">
              Click the SYNC button in the navbar to run the pipeline and generate your first score.
            </p>
            <button onClick={() => mutate()} className="px-4 py-2 rounded-md border border-border bg-card text-muted-foreground font-mono text-sm hover:bg-accent hover:text-foreground transition-all">
              Retry
            </button>
          </div>
        </main>
        ...
        <main className="max-w-7xl mx-auto px-4 py-8 w-full">
          ...
          {snapshot && !snapshot.assessment?.overall_score && (
            <div className="mb-6 p-4 rounded-lg border border-primary/30 bg-primary/5 text-primary text-sm font-mono flex items-center gap-2">
              <span className="text-lg">⚠</span> No market data yet — add a RapidAPI key to enable job matching
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            ...
            <div className="glass-panel p-5 rounded-xl flex flex-col h-full relative overflow-hidden group">
              ...
              <div className="flex items-center gap-2 mb-4 relative z-10">
                ...
                <h2 className="text-sm font-mono font-semibold text-foreground uppercase tracking-wider">GitHub Activity</h2>
              </div>
              ...
              {snapshot?.github_data ? (
                <>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 relative z-10 w-full mb-5">
                    {[
                      { label: 'REPOS',        value: snapshot.github_data.public_repos,                              color: 'text-foreground' },
                      { label: 'COMMITS (90D)', value: snapshot.github_data.contribution_stats.total_commits_last_90d, color: 'text-green-500' },
                      { label: 'STREAK',        value: `${snapshot.github_data.contribution_stats.contribution_streak}d`, color: 'text-primary' },
                      { label: 'PRs',           value: snapshot.github_data.contribution_stats.total_prs,             color: 'text-foreground' },
                    ].map((stat) => (
                      <div key={stat.label} className="p-3 rounded-lg bg-background/50 border border-border/50 text-center hover:bg-muted/50 transition-colors">
                        <p className="text-[10px] font-mono text-muted-foreground mb-1 whitespace-nowrap overflow-hidden text-ellipsis">{stat.label}</p>
                  ...
                  <div className="relative z-10 border-t border-border/50 pt-4">
                    ...
                    <RadarCoverageChart snapshot={snapshot} skillTrends={skillTrends} />
                  </div>
                </>
              ) : (
                <p className="text-sm font-mono text-muted-foreground relative z-10">No GitHub data available</p>
              )}
            </div>
            ...
            <div className="glass-panel p-5 rounded-xl flex flex-col h-full relative overflow-hidden group">
              ...
              <div className="flex items-center gap-2 mb-4 relative z-10">
                ...
                <h2 className="text-sm font-mono font-semibold text-foreground uppercase tracking-wider">Skill Market Trends</h2>
              </div>
              <div className="relative z-10 w-full overflow-hidden">
                {skillTrends
                  ? <SkillHeatmap data={skillTrends.trends} topSkills={skillTrends.top_trending} />
                  : <p className="text-sm font-mono text-muted-foreground">No trend data available</p>
                }
              </div>
            </div>
            ...
            <div className="glass-panel p-5 rounded-xl flex flex-col h-full relative overflow-hidden group">
              ...
              <div className="flex items-center gap-2 mb-4 relative z-10">
                ...
                <h2 className="text-sm font-mono font-semibold text-foreground uppercase tracking-wider">Priority Skill Gaps</h2>
              </div>
              <div className="relative z-10 w-full overflow-hidden">
                {gaps
                  ? <GapList gaps={gaps.gaps.slice(0, 5)} />
                  : <p className="text-sm font-mono text-muted-foreground">No gaps identified</p>
                }
              </div>
            </div>
            ...
            {snapshot?.assessment?.strengths && (
              <div className="glass-panel p-5 rounded-xl border-green-500/20 bg-green-500/5 flex flex-col h-full relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 mb-4 relative z-10">
                  ...
                  <h2 className="text-sm font-mono font-semibold text-green-500 uppercase tracking-wider">Your Strengths</h2>
                </div>
                <ul className="flex flex-col gap-3 relative z-10 w-full">
                  {snapshot.assessment.strengths.map((strength, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground group/item">
                      ...
                      <span className="font-mono group-hover/item:text-foreground transition-colors">{strength}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {snapshot?.assessment?.top_matching_jobs && snapshot.assessment.top_matching_jobs.length > 0 && (
              <div className="glass-panel p-5 rounded-xl flex flex-col col-span-1 lg:col-span-2 relative overflow-hidden">
                <h2 className="text-sm font-mono font-semibold text-secondary uppercase mb-4 tracking-wider relative z-10">Top Matches</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
                  {snapshot.assessment.top_matching_jobs.slice(0, 3).map((job) => (
                    <div key={job.job_id} className="flex flex-col gap-3 p-4 rounded-lg bg-background/50 border border-border/50 hover:border-secondary/30 hover:bg-secondary/5 transition-all cursor-pointer group hover:-translate-y-0.5">
                      <span className="text-sm font-display font-medium text-muted-foreground group-hover:text-foreground transition-colors truncate">{job.job_id.slice(0, 20)}…</span>
                      <div className="flex items-center justify-between mt-auto">
                        <span className="text-xs font-mono text-muted-foreground">Match:</span>
                        <span className={`text-xs font-mono px-2 py-1 rounded-md font-bold ${
                          job.match_pct >= 80 ? 'bg-green-500/10 text-green-500 border border-green-500/20' :
                          job.match_pct >= 60 ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' :
            ...
            {snapshot?.assessment?.weekly_recommendation && (
              <div className="glass-panel p-6 rounded-xl border border-primary/20 bg-primary/5 flex flex-col col-span-1 lg:col-span-2 relative overflow-hidden shadow-glow-primary">
                ...
                <div className="flex items-center gap-2 mb-4 relative z-10">
                  ...
                  <h2 className="text-sm font-mono font-semibold text-primary uppercase tracking-wider">Weekly Recommendation</h2>
                </div>
