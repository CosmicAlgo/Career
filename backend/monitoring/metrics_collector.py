...
class MetricsCollector:
    ...
    async def start_pipeline_run(self) -> str:
        ...
        async with self._lock:
            self.current_run = PipelineMetrics(
                run_id=run_id, start_time=datetime.utcnow()
            )
    ...
    async def record_scraping_duration(
        self, duration_ms: int, jobs_by_source: Dict[str, int]
    ):
        ...
        if self.current_run:
            ...
            logger.info(
                f"[Metrics] Scraping: {duration_ms}ms, {self.current_run.jobs_scraped} jobs"
            )

    async def record_scraper_metrics(
        self,
        name: str,
        duration_ms: int,
        jobs_returned: int,
        ...
        if error:
            logger.warning(
                f"[Metrics] Scraper {name}: {duration_ms}ms, {jobs_returned} jobs, ERROR: {error}"
            )
        else:
            logger.info(
                f"[Metrics] Scraper {name}: {duration_ms}ms, {jobs_returned} jobs"
            )
    ...
    async def finish_pipeline_run(self, status: str = "success"):
        ...
        logger.info(
            f"[Metrics] Pipeline run finished: {status}, total: {self.current_run.total_duration_ms}ms"
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "jobs": {"total": run.jobs_scraped, "by_source": run.jobs_by_source},
            "scrapers": [
                {
                    "name": s.name,
