...
class NormalisedJob(BaseModel):
    ...
    id: str = Field(..., description="Unique job identifier (hash of title+company+date)")
...
class SkillGap(BaseModel):
    ...
    frequency_in_market: int = Field(..., description="How often mentioned in job postings")
...
class AssessmentResult(BaseModel):
    ...
    strengths: List[str] = Field(
        default_factory=list,
        description="Candidate strengths from GitHub analysis"
    )
    weekly_recommendation: str = Field(
        default="",
        description="Weekly actionable recommendation"
    )
...
class RefreshRequest(BaseModel):
    ...
    force: bool = Field(default=False, description="Force refresh even if already ran today")
