...
from api.schemas import NormalisedJob, GitHubSummary, AssessmentResult
from assessment.skill_embedder import (
    embed_profile, embed_job, batch_embed_jobs,
    extract_skills_from_job, extract_skills_from_profile
)
...
async def compute_local_ml_scores(
    ...
    logger.info(f"[ML Scorer] Top 5 job matches: {sorted(job_scores.items(), key=lambda x: x[1], reverse=True)[:5]}")
