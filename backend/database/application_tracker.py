...
class ApplicationTracker:
    ...
    async def create_application(self, application_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            application_data.update({
                ...
                "follow_up_date": (datetime.utcnow() + timedelta(days=7)).isoformat() if application_data.get("status") == "applied" else None
            })
            
            response = self.client.client.table("job_applications").insert(application_data).execute()
    ...
    async def get_applications(self, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get all job applications, optionally filtered by user."""
        try:
            query = self.client.client.table("job_applications").select("*").order("created_at", desc=True)
    ...
    async def update_application_status(self, application_id: str, status: str, notes: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if status == "applied":
                update_data["follow_up_date"] = (datetime.utcnow() + timedelta(days=7)).isoformat()
            elif status in ["offer", "rejected"]:
                update_data["follow_up_date"] = None
            
            response = self.client.client.table("job_applications").update(update_data).eq("id", application_id).execute()
    ...
    async def get_applications_by_status(self, status: str) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.client.client.table("job_applications").select("*").eq("status", status).order("created_at", desc=True).execute()
            return response.data or []
    ...
    async def get_follow_ups_due(self, days_ahead: int = 0) -> List[Dict[str, Any]]:
        ...
        try:
            target_date = (datetime.utcnow() + timedelta(days=days_ahead)).date().isoformat()
            
            response = self.client.client.table("job_applications").select("*").eq("follow_up_date", target_date).order("follow_up_date", desc=True).execute()
            return response.data or []
    ...
    async def get_application_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.client.client.table("job_applications").select("*").gte("created_at", cutoff_date).execute()
            ...
            for app in applications:
                ...
                # Calculate rates
                if status in ["phone_screen", "technical", "final", "offer", "rejected"]:
                    responses += 1
                    if app.get("response_date"):
                        response_days.append(self._calculate_days_between(app["created_at"], app["response_date"]))
            ...
            if stats["total_applications"] > 0:
                ...
                stats["interview_rate"] = (interviews / stats["total_applications"]) * 100
