...
class CVProcessor:
    ...
    def __init__(self):
        ...
        self.nlp = None
        print("[CVProcessor] Using regex-based extraction (spaCy disabled for Python 3.14 compatibility)")
        
        # Common skill keywords for tech roles
        self.tech_skills = {
            'languages': ['python', 'javascript', 'typescript', 'java', 'c++', 'c#', 'go', 'rust', 'swift', 'kotlin'],
            'frameworks': ['react', 'vue', 'angular', 'django', 'flask', 'fastapi', 'spring', 'node.js', 'express', 'next.js'],
            'databases': ['postgresql', 'mysql', 'mongodb', 'redis', 'elasticsearch', 'sqlite', 'oracle', 'sql server'],
            'cloud': ['aws', 'azure', 'gcp', 'google cloud', 'terraform', 'kubernetes', 'docker', 'jenkins', 'github actions'],
            'ml': ['tensorflow', 'pytorch', 'scikit-learn', 'pandas', 'numpy', 'jupyter', 'keras', 'huggingface', 'langchain'],
            'tools': ['git', 'linux', 'ubuntu', 'docker', 'kubernetes', 'nginx', 'apache', 'microservices', 'rest api']
        }
        ...
        self.education_keywords = {
            ...
            'certifications': ['certificate', 'certification', 'aws certified', 'google certified']
        }
    ...
    def _analyze_cv_text(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'cv_score': cv_score,
            'raw_text': text[:1000] + "..." if len(text) > 1000 else text  # Store snippet
        }
            ...
            for keyword in keywords:
                if keyword in education_text.lower():
                    ...
                    education.append({
                        ...
                        'text': education_text[:200] + "..." if len(education_text) > 200 else education_text
                    })
                    break
    ...
    def _calculate_cv_score(self, skills: Dict[str, List[str]], experience: List[Dict], education: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'breakdown': {
                'skills': skills_score,
                'experience': 35 if len(experience) >= 2 else (25 if len(experience) >= 1 else 0),
                'education': 25 if 'phd' in education_levels or 'masters' in education_levels else (20 if 'bachelors' in education_levels else (15 if 'certifications' in education_levels else 0))
            }
        }
...
class CVMatcher:
    ...
    async def analyze_cv_against_market(self, cv_data: Dict[str, Any], target_roles: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            role_scores = self._calculate_role_scores(cv_data['skills'], target_roles, market_data)
            
            # Generate improvement suggestions
            suggestions = self._generate_suggestions(cv_data, skill_analysis, role_scores)
            ...
            return {
                ...
                'market_match': self._calculate_market_match(cv_data['skills'], market_data),
    ...
    async def _get_market_data(self, target_roles: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = db.table('jobs').select('*').gte('posted_date', cutoff_date).execute()
            ...
            for job in jobs:
                ...
                for target_role in target_roles:
                    if target_role.replace('_', ' ') in job_role or target_role in job_role:
            ...
            return {
                ...
                'market_skills': dict(sorted(market_skills.items(), key=lambda x: x[1], reverse=True)[:50]),
            ...
            return {'total_jobs_analyzed': 0, 'market_skills': {}, 'role_specific_jobs': {}}
    ...
    def _analyze_skill_gaps(self, cv_skills: Dict[str, List[str]], market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for skill, frequency in market_skills.items():
            if skill.lower() not in cv_skill_set and frequency >= 5:  # Skills appearing in 5+ jobs
                missing_skills.append({
                    'skill': skill,
                    'market_frequency': frequency,
                    'priority': 'high' if frequency >= 20 else 'medium' if frequency >= 10 else 'low'
                })
        ...
        return {
            ...
            'in_demand_skills': len([s for s in cv_skill_list if s.lower() in [ms.lower() for ms in market_skills.keys()]]),
            'missing_skills': sorted(missing_skills, key=lambda x: x['market_frequency'], reverse=True)[:10],
            'declining_skills': declining_skills[:5],
            'skill_coverage': len([s for s in cv_skill_list if s.lower() in [ms.lower() for ms in market_skills.keys()]]) / max(len(market_skills), 1) * 100
        }
    
    async def _compare_with_github(self, cv_data: Dict[str, Any], target_roles: List[str]) -> Dict[str, Any]:
        """Compare CV skills with GitHub profile."""
        try:
            # Get user settings to find GitHub username
            db = supabase_manager.client
            settings_response = db.table('user_settings').select('*').order('updated_at', desc=True).limit(1).execute()
            
            if not settings_response.data:
                return {'github_available': False, 'message': 'No GitHub username found'}
    ...
    def _calculate_role_scores(self, cv_skills: Dict[str, List[str]], target_roles: List[str], market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for role in target_roles:
            ...
            if not jobs:
                role_scores[role] = {'score': 0, 'matched_skills': [], 'missing_skills': [], 'total_jobs': 0}
                continue
            
            # Extract required skills from job descriptions
            all_job_skills = []
            for job in jobs:
                all_job_skills.extend(self._extract_job_skills(job.get('description', '')))
            ...
            score_percentage = (matched_count / top_skills_count) * 100 if top_skills_count > 0 else 0
    ...
    def _calculate_market_match(self, cv_skills: Dict[str, List[str]], market_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_suggestions(self, cv_data: Dict[str, Any], skill_analysis: Dict[str, Any], role_scores: Dict[str, Any]) -> List[str]:
        ...
        if missing_skills:
            suggestions.append(f"Consider adding these in-demand skills: {', '.join([s['skill'] for s in missing_skills])}")
        
        # Add role-specific suggestions
        low_scoring_roles = [role for role, data in role_scores.items() if data['score'] < 50]
        if low_scoring_roles:
            suggestions.append(f"Your CV needs improvement for: {', '.join(low_scoring_roles)}")
        
        # Add positive feedback
        high_scoring_roles = [role for role, data in role_scores.items() if data['score'] >= 70]
