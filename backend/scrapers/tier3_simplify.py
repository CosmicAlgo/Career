...
class SimplifyScraper(BaseScraper):
    ...
    async def scrape(
        ...
        try:
            ...
            if not job_cards:
                job_cards = soup.find_all("a", href=lambda x: x and "/jobs/" in x if x else False)
    ...
    def _parse_job_card(self, card) -> Optional[RawJobListing]:
        ...
        title = self._extract_text(card, ["h2", "h3", "h4", ".title", "[data-testid='job-title']", "a"])
        company = self._extract_text(card, [".company", "[data-testid='company']", ".employer"])
        ...
        posted_text = self._extract_text(card, [".date", ".posted", "[data-testid='date']"])
