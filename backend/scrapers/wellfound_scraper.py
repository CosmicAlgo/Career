    ...
    async def scrape(
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                for role in target_roles[:3]:  # Limit to first 3 roles to avoid timeouts
                    ...
                    try:
                        ...
                        job_cards = await page.query_selector_all("[data-test='job-listing']")
                        
                        for card in job_cards[:10]:  # Limit per role
                            try:
                                # Extract job details
                                title_el = await card.query_selector("h2, .title, [data-test='job-title']")
                                company_el = await card.query_selector(".company-name, [data-test='company-name']")
                                location_el = await card.query_selector(".location, [data-test='location']")
                                ...
                                logger.warning(f"[Wellfound] Error extracting job card: {e}")
                                continue
                        ...
                        logger.warning(f"[Wellfound] Error searching for role {role}: {e}")
                        continue
