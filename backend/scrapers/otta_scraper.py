...
class OttaScraper(BaseScraper):
    ...
    def _title_matches_roles(self, title: str, target_roles: List[str]) -> bool:
        ...
        role_keywords = {
            "ml_engineer": ["ml engineer", "machine learning", "ml developer", "ai engineer", "ai/ml"],
            ...
            "backend": ["backend", "back-end", "server-side", "api developer", "software engineer", "fullstack"],
            "data_engineer": ["data engineer", "data eng", "etl", "data pipeline", "data infrastructure"],
    ...
    async def scrape(
        ...
        try:
            logger.info(f"[Otta] Starting scrape for roles: {target_roles} (UK focused: {uk_focused})")
            ...
            async with async_playwright() as p:
                ...
                query_roles = " OR ".join([r.replace("_", " ") for r in target_roles[:2]])
                ...
                try:
                    cookie_btn = await page.query_selector("button:has-text('Accept'), button:has-text('accept'), [data-test='accept-cookies']")
                    if cookie_btn:
                ...
                job_cards = await page.query_selector_all("article, [data-testid='job-card'], .sc-" + "job-card")
                ...
                for card in job_cards[:max_results]:
                    try:
                        ...
                        title_el = await card.query_selector("h2, h3, .title, [data-testid='job-title']")
                        if not title_el:
                            continue
                        ...
                        # Extract company
                        company_el = await card.query_selector(".company-name, [data-testid='company-name'], .sc-company")
                        company = await company_el.inner_text() if company_el else None
                        
                        # Extract location
                        location_el = await card.query_selector(".location, [data-testid='location'], .sc-location")
                        location = await location_el.inner_text() if location_el else "UK"
                        
                        # Check UK focus
                        if uk_focused and not any(loc in location.lower() for loc in ["uk", "london", "england", "united kingdom", "remote"]):
                            continue
                        
                        # Extract salary if available
                        salary_el = await card.query_selector(".salary, [data-testid='salary'], .sc-salary")
                        salary_text = await salary_el.inner_text() if salary_el else ""
                        salary_min, salary_max = self._extract_salary(salary_text) if salary_text else (None, None)
                        ...
                        if link_el:
                            ...
                            if href:
                                job_url = f"https://app.welcometothejungle.com{href}" if href.startswith("/") else href
                        ...
                        if "senior" in title_lower or "lead" in title_lower or "principal" in title_lower:
                            seniority = "senior"
                        elif "junior" in title_lower or "graduate" in title_lower or "entry" in title_lower:
                            seniority = "junior"
                        
                        # Check remote
                        remote = "remote" in location.lower() or "hybrid" in location.lower() or "flexible" in location.lower()
