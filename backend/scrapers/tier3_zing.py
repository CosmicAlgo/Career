...
class ZingScraper(BaseScraper):
    ...
    def __init__(self):
        super().__init__(
            tier=ScraperTier.TIER_3_DIRECT,
            max_retries=2,
            retry_delay=2.0
        )
    ...
    async def scrape(
        ...
        try:
            ...
            job_cards = (
                ...
                soup.find_all("div", class_=lambda x: x and "job" in str(x).lower() if x else False) or
                soup.find_all("a", href=lambda x: x and "/job/" in x if x else False)
            )
    ...
    def _parse_job_card(self, card) -> Optional[RawJobListing]:
        ...
        description = self._extract_text(card, [
            ...
            ".summary", ".snippet", "[data-testid='description']"
        ])
        
        # Try to find salary info
        salary_text = self._extract_text(card, [
        ...
        # Parse posted date
        date_text = self._extract_text(card, [
            ...
            try:
                if hasattr(element, 'select_one'):
                    ...
                    if selector.startswith('.') or selector.startswith('['):
                        ...
                        found = element.find(class_=lambda x: class_name in str(x) if x else False)
                    else:
