...
class JSearchScraper(BaseScraper):
    ...
    async def scrape(
        ...
        logger.info(f"[JSearch] Built {len(queries)} queries from {len(target_roles)} roles and {len(locations)} locations")
        ...
        try:
            ...
            for job in unique_jobs:
                ...
                normalised_job = NormalisedJob(
                    ...
                    location=job.get("job_location") or f"{job.get('job_city', '')}, {job.get('job_country', '')}".strip(", "),
