...
def time_bucket(
    timestamps: List[datetime], values: List[float], bucket_size: str = "1 day"
    ...
    for d in all_dates:
        if d in data_dict:
            ...
            if before and after:
                ...
                result.append(
                    TimeSeriesPoint(
                        date=d, value=round(interpolated_val, 2), is_interpolated=True
                    )
                )
            elif before:
                ...
                result.append(
                    TimeSeriesPoint(date=d, value=last_val, is_interpolated=True)
                )
            elif after:
                ...
                result.append(
                    TimeSeriesPoint(date=d, value=first_val, is_interpolated=True)
                )
...
def calculate_rolling_average(
    data: List[TimeSeriesPoint], window_days: int = 7
) -> List[TimeSeriesPoint]:
    """
...
def calculate_trend(data: List[TimeSeriesPoint], window_days: int = 7) -> float:
    """
...
def format_for_chart(
    data: List[TimeSeriesPoint], include_interpolated: bool = True
) -> List[Dict[str, Any]]:
    """
...
def score_history_to_timeseries(
    snapshots: List[Dict[str, Any]], role: Optional[str] = None, gap_fill: bool = True
) -> List[TimeSeriesPoint]:
    """
    ...
    for snap in snapshots:
        ...
        points.append(
            TimeSeriesPoint(date=snap_date, value=float(score), is_interpolated=False)
        )
...
class TrendAnalyzer:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "days_tracked": (self.data[-1].date - self.data[0].date).days
            if len(self.data) > 1
            else 0,
        }
    ...
    def detect_anomalies(self, threshold_std: float = 2.0) -> List[TimeSeriesPoint]:
        """
