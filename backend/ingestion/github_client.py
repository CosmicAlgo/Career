...
class GitHubClient:
    ...
    def _extract_certifications(self, readme_text: Optional[str], repos: List[Dict]) -> List[str]:
        ...
        cert_keywords = [
            ...
            "Certified Kubernetes", "CKA", "CKAD", "AWS Solutions Architect",
            "AWS Developer", "AWS SysOps", "Microsoft Certified", "Oracle Certified",
            "CompTIA", "CCNA", "CCNP", "PMP", "Scrum Master", "CSM"
        ]
    ...
    async def build_summary(self) -> GitHubSummary:
        ...
        top_repos = []
        for repo in sorted(public_repos, key=lambda x: x.get("stargazerCount", 0), reverse=True)[:5]:
            last_commit = None
        ...
        readme_excerpt = None
        if readme_content:
            readme_excerpt = readme_content[:2000] + "..." if len(readme_content) > 2000 else readme_content
