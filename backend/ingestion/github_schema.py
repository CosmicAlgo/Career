...
class RepoInfo(BaseModel):
    ...
    last_commit: Optional[str] = Field(
        default=None, description="Last commit date (ISO format)"
    )
...
class ContributionStats(BaseModel):
    ...
    total_commits_last_90d: int = Field(
        default=0, description="Total commits in last 90 days"
    )
    contribution_streak: int = Field(
        default=0, description="Current contribution streak"
    )
...
class GitHubSummary(BaseModel):
    ...
    languages: Dict[str, float] = Field(
        default_factory=dict, description="Language breakdown (name -> percentage)"
    )
    top_repos: List[RepoInfo] = Field(
        default_factory=list, description="Top repositories by stars"
    )

    contribution_stats: ContributionStats = Field(
        default_factory=ContributionStats, description="Contribution statistics"
    )

    readme_excerpt: Optional[str] = Field(
        default=None, description="Excerpt from profile README"
    )
    certifications_detected: List[str] = Field(
        default_factory=list, description="Certifications detected in README/repos"
    )
